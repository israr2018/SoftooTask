...
const transactionsFilePath: string =
    env.TRANSACTIONS_FILE_PATH || 'data/transactions2.json'
...
function calculatStockLevel(acc: number, current: ITransactions): number {
    if (current.type === 'order') return acc + current.qty
    else return acc - current.qty
}
